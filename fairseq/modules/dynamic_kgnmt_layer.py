...
class KgNMTDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        ### TODO_THESIS: ADDED, have to check size here
        ############################################################
        if self.knw_encoder_attn is not None and knw_encoder_out is not None:
            residual = x
            if self.normalize_before:
            ...
            if prev_attn_state is not None:
                ...
                assert incremental_state is not None
                self.knw_encoder_attn._set_input_buffer(incremental_state, saved_state)

            x, attn = self.knw_encoder_attn(
                query=x,
                key=knw_encoder_out,
                value=knw_encoder_out,
                key_padding_mask=knw_encoder_padding_mask,
                incremental_state=incremental_state,
                static_kv=True,
