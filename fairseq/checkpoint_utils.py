...
# def _upgrade_state_dict(state): # TODO_THESIS: some error about best_loss here
...
def _upgrade_state_dict(state): # TODO_THESIS: some error about best_loss here
    ...
    # add optimizer_history
    if "optimizer_history" not in state and "kgnmt_optimizer_history" not in state and "knowledge_selector_optimizer_history" not in state:
        state["optimizer_history"] = [
            {"criterion_name": "CrossEntropyCriterion", "best_loss": state["best_loss"]}
        ]
        state["last_optimizer_state"] = state["optimizer"]
    ...
    if "epoch" in state and "extra_state" not in state:
        ...
        del state["val_loss"]
    # reduce optimizer history's memory usage (only keep the last state)
    if "optimizer" in state["kgnmt_optimizer_history"][-1] and "optimizer" in state["knowledge_selector_optimizer_history"][-1]:
        state["last_kgnmt_optimizer_state"] = state["kgnmt_optimizer_history"][-1]["optimizer"]
        state["last_knowledge_selector_optimizer_state"] = state["knowledge_selector_optimizer_history"][-1]["optimizer"]
        for optim_hist in state["kngmt_optimizer_history"]:
            del optim_hist["optimizer"]
        for optim_hist in state["knowledge_selector_optimizer_history"]:
            del optim_hist["optimizer"]
    # record the optimizer class name
    if "optimizer_name" not in state["kgnmt_optimizer_history"][-1] and "optimizer_name" not in state["knowledge_selector_optimizer_history"][-1]:
        state["kgnmt_optimizer_history"][-1]["optimizer_name"] = "FairseqNAG"
        state["knowledge_selector_optimizer_history"][-1]["optimizer_name"] = "FairseqNAG"
    # move best_loss into lr_scheduler_state
    if "lr_scheduler_state" not in state["kgnmt_optimizer_history"][-1] and "lr_scheduler_state" not in state["knowledge_selector_optimizer_history"][-1]:
        state["kgnmt_optimizer_history"][-1]["lr_scheduler_state"] = {
            "best": state["kgnmt_optimizer_history"][-1]["best_loss"]
        }
        del state["kgnmt_optimizer_history"][-1]["best_loss"]
        state["knowledge_selector_optimizer_history"][-1]["lr_scheduler_state"] = {
            "best": state["knowledge_selector_optimizer_history"][-1]["best_loss"]
        }
        del state["knowledge_selector_optimizer_history"][-1]["best_loss"]
    # keep track of number of updates
    if "num_updates" not in state["kgnmt_optimizer_history"][-1] and "num_updates" not in state["knowledge_selector_optimizer_history"][-1]:
        state["kgnmt_optimizer_history"][-1]["num_updates"] = 0
        state["knowledge_selector_optimizer_history"][-1]["num_updates"] = 0
    # use stateful training data iterator
    if "train_iterator" not in state["extra_state"]:
        state["extra_state"]["train_iterator"] = {
            "epoch": state["extra_state"].get("epoch", 0),
