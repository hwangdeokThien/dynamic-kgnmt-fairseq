...
class SequenceGeneratorKnowledgeAug(nn.Module):
    def __init__(
        self,
        models,
        tgt_dict,
    ...
    def generate_batched_itr(self, data_itr, beam_size=None, cuda=False, timer=None):
        ...
        for sample in data_itr:
            ...
            if "net_input" not in s:
                continue
            model_input = s["net_input"]

            # model.forward normally channels prev_output_tokens into the decoder
            # separately, but SequenceGenerator directly calls model.encoder
            both_encoder_input = {
                k: v for k, v in model_input.items() if k != "prev_output_tokens"
            }
            if timer is not None:
                timer.start()
            with torch.no_grad():
                hypos = self.generate(both_encoder_input)
            if timer is not None:
                timer.stop(sum(len(h[0]["tokens"]) for h in hypos))
            for i, id in enumerate(s["id"].data):
                # remove padding
                src = utils.strip_pad(model_input["src_tokens"].data[i, :], self.pad)
                ref = (
    ...
    # TODO_THESIS: maybe have to modify this so the evaluate function can accept knowledge as an component
    def _generate(
        self,
        sample: Dict[str, Dict[str, Tensor]],
        prefix_tokens: Optional[Tensor] = None,
        ...
        knw_encoder_outs = self.model.reorder_encoder_out(knw_encoder_outs, new_order)
        # ensure encoder_outs is a List.
        assert encoder_outs is not None
        assert knw_encoder_outs is not None
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        knw_encoder_outs: List[Dict[str, List[Tensor]]],
